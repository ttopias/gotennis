...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	startTotal := time.Now()
	log.Printf("Received request from %s: p1=%f, p2=%f, bestof=%d, simulations=%d", r.RemoteAddr, p1, p2, bestof, simulations)
	start := time.Now()
	...
	res, err := deriveProbabilities(sim, bestof)
	if err != nil {
		stat.Success = 0
		stat.Error = 1
		addRequestStat(stat)
	...
	log.Printf("With p1=%f, p2=%f, bestof=%d - ML probs: %f, %f", p1, p2, bestof, res.Moneyline.ProbA, res.Moneyline.ProbB)
...
func deriveProbabilities(match []sim.SimulatedMatch, bestof int) (SimulationResult, error) {
...
func validateInputs(p1, p2 float64, bestof int, err1, err2, err3 error) error {
	if err1 != nil || err2 != nil || err3 != nil {
		return fmt.Errorf("Invalid query parameters: parse error")
	}
	if bestof != 3 && bestof != 5 {
		return fmt.Errorf("Invalid bestof value: must be 3 or 5")
	}
