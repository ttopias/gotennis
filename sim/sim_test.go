...
func TestSimulateGame(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.GreaterOrEqual(t, result, tt.expectedMin, "simulateGame(%f) = %f, expected >= %f (%s)", tt.p, result, tt.expectedMin, tt.description)
			assert.LessOrEqual(t, result, tt.expectedMax, "simulateGame(%f) = %f, expected <= %f (%s)", tt.p, result, tt.expectedMax, tt.description)
...
func TestSimulateGameMathematicalProperties(t *testing.T) {
	...
	for _, p := range testCases {
		t.Run(fmt.Sprintf("p=%.1f", p), func(t *testing.T) {
			...
			assert.InDelta(t, result, result2, 1e-10, "simulateGame should be deterministic, got %f and %f", result, result2)
		})
	}
}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < simulations; i++ {
				...
				assert.True(t, isValidSetScore(result.AGames, result.BGames), "invalid set score: %d-%d", result.AGames, result.BGames)
				if result.AGames > result.BGames {
					aWins++
				}
			}
			if tt.a > tt.b+0.2 {
				assert.GreaterOrEqual(t, float64(aWins)/float64(simulations), 0.6, "warning: Strong player A only won %d%% of sets against weak B", int(100*float64(aWins)/float64(simulations)))
			}
		})
	}
}
...
func TestAWinsTiebreak(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.a > tt.b+0.2 {
				assert.Greater(t, winRate, 0.6, "strong player A should win >60%% of tiebreaks, got %.1f%%", winRate*100)
			}
			if tt.a < tt.b-0.2 {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < tt.iterations; i++ {
				...
				assert.True(t, result.ASets == tt.setsToWin || result.BSets == tt.setsToWin, "match should end when someone reaches %d sets, got A=%d, B=%d", tt.setsToWin, result.ASets, result.BSets)
				assert.Equal(t, len(result.SetResults), result.ASets+result.BSets, "number of set results (%d) should equal total sets (%d)", len(result.SetResults), result.ASets+result.BSets)
				for j, set := range result.SetResults {
					assert.True(t, isValidSetScore(set.AGames, set.BGames), "invalid set score in set %d: %d-%d", j, set.AGames, set.BGames)
				}
			...
			if tt.pA > tt.pB+0.3 && tt.iterations > 50 {
				assert.Greater(t, winRate, 0.4, "strong player A should win >40%% of matches with large sample, got %.1f%%", winRate*100)
			}
		})
	}
}
...
func TestSimulateMatchIntegration(t *testing.T) {
	t.Run("Small scale integration test", func(t *testing.T) {
		...
		assert.GreaterOrEqual(t, len(result.SetResults), 2, "bO3 match should have at least 2 sets, got %d", len(result.SetResults))
		assert.LessOrEqual(t, len(result.SetResults), 3, "bO3 match should have at most 3 sets, got %d", len(result.SetResults))
	})
}
...
func TestSimulateGameEdgeCases(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.GreaterOrEqual(t, result, 0.0, "simulateGame(%f) = %f, should be >= 0", tt.p, result)
			assert.LessOrEqual(t, result, 1.0, "simulateGame(%f) = %f, should be <= 1", tt.p, result)
			assert.False(t, math.IsNaN(result) || math.IsInf(result, 0), "simulateGame(%f) = %f, should not be NaN or Inf", tt.p, result)
		})
	}
}
...
func TestAgainstDiscoverMD(t *testing.T) {
	t.Run("GameWinner_p70_yields_90_percent_win", func(t *testing.T) {
		...
		assert.InDeltaf(t, expectedGameWinProb, actual, tolerance, "simulateGame(%.2f): expected ~%.2f (within %.2f), got %.4f", p, expectedGameWinProb, tolerance, actual)
	})
	...
	t.Run("SetWinner_p60_vs_p50_yields_80_percent_win", func(t *testing.T) {
		...
		actualWinRate := float64(aWins) / float64(N_SIMS)
		assert.InDeltaf(t, expectedWinRate, actualWinRate, tolerance, "set win rate for %.2f vs %.2f: expected ~%.2f (within %.2f), got %.4f", pA, pB, expectedWinRate, tolerance, actualWinRate)
	})
	...
	t.Run("SetWinner_8pp_diff_yields_76_percent_win", func(t *testing.T) {
		...
		actualWinRate := float64(aWins) / float64(N_SIMS)
		assert.InDeltaf(t, expectedWinRate, actualWinRate, tolerance, "set win rate for %.2f vs %.2f (8pp diff): expected ~%.2f (within %.2f), got %.4f", pA, pB, expectedWinRate, tolerance, actualWinRate)
	})
	...
	t.Run("MatchWinner_8pp_diff_yields_83_percent_win_BO3", func(t *testing.T) {
		...
		actualWinRate := float64(aWins) / float64(N_SIMS)
		assert.InDeltaf(t, expectedWinRate, actualWinRate, tolerance, "bO3 Match win rate for %.2f vs %.2f (8pp diff): expected ~%.2f (within %.2f), got %.4f", pA, pB, expectedWinRate, tolerance, actualWinRate)
	})
	...
	t.Run("MatchWinner_4pp_diff_yields_70_percent_win_BO3", func(t *testing.T) {
		...
		actualWinRate := float64(aWins) / float64(N_SIMS)
		assert.InDeltaf(t, expectedWinRate, actualWinRate, tolerance, "bO3 Match win rate for %.2f vs %.2f (4pp diff): expected ~%.2f (within %.2f), got %.4f", pA, pB, expectedWinRate, tolerance, actualWinRate)
	})
}
