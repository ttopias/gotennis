...
func simulateGame(p float64) float64 {
	...
	denominator_deuce := 1 - 2*p*(1-p)
	if math.Abs(denominator_deuce) < 1e-10 {
		...
		p_deuce = (p * p) / denominator_deuce
		p_deuce = math.Max(0.001, math.Min(0.999, p_deuce))
	}

	// P(win 4-0): p^4
	prob_4_0 := math.Pow(p, 4)

	// P(win 4-1): 4 ways to lose 1 point in first 4 (p^3*(1-p)), then win next (p). So 4 * p^4 * (1-p)
	prob_4_1 := 4 * math.Pow(p, 4) * (1 - p)
	// P(win 4-2): 10 ways to lose 2 points in first 5 (C(5,2)=10 * p^3*(1-p)^2), then win next (p).
	prob_4_2 := 10 * math.Pow(p, 4) * math.Pow(1-p, 2)
	...
	prob_reach_deuce := 20 * math.Pow(p, 3) * math.Pow(1-p, 3)
	prob_win_from_deuce := prob_reach_deuce * p_deuce

	return prob_4_0 + prob_4_1 + prob_4_2 + prob_win_from_deuce
}
