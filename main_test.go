...
func TestHandler(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if !tt.expectError {
				url := fmt.Sprintf("/?%s", tt.queryParams)
			...
			if tt.expectError {
				...
				assert.False(t, contentType == "application/json" && w.Code >= 400, "Error response should not have JSON content type")
			}
		})
	}
}
...
func validateSimulationResponse(t *testing.T, result Simulation) {
	assert.GreaterOrEqual(t, result.P1, 0.0, "P1 should be >= 0, got %f", result.P1)
	assert.LessOrEqual(t, result.P1, 1.0, "P1 should be <= 1, got %f", result.P1)
	assert.GreaterOrEqual(t, result.P2, 0.0, "P2 should be >= 0, got %f", result.P2)
	assert.LessOrEqual(t, result.P2, 1.0, "P2 should be <= 1, got %f", result.P2)

	sr := result.SimulationResult
	assert.Equal(t, format.Moneyline, sr.Moneyline.Market, "Expected moneyline market to be %s, got %s", format.Moneyline, sr.Moneyline.Market)
	assert.NotEmpty(t, sr.SetHandicaps, "SetHandicaps should not be empty")
	for i, sh := range sr.SetHandicaps {
		assert.Equal(t, format.Handicap, sh.Market, "SetHandicaps[%d] should have market %s, got %s", i, format.Handicap, sh.Market)
		validateProbability(t, fmt.Sprintf("SetHandicaps[%d]", i), sh)
	}
	assert.NotEmpty(t, sr.GameHandicaps, "GameHandicaps should not be empty")
	for i, gh := range sr.GameHandicaps {
		assert.Equal(t, format.Handicap, gh.Market, "GameHandicaps[%d] should have market %s, got %s", i, format.Handicap, gh.Market)
		validateProbability(t, fmt.Sprintf("GameHandicaps[%d]", i), gh)
	}
	assert.NotEmpty(t, sr.SetOU, "SetOU should not be empty")
	for i, st := range sr.SetOU {
		assert.Equal(t, format.Total, st.Market, "SetOU[%d] should have market %s, got %s", i, format.Total, st.Market)
		validateProbability(t, fmt.Sprintf("SetOU[%d]", i), st)
	}
	assert.NotEmpty(t, sr.GameOU, "GameOU should not be empty")
	for i, gt := range sr.GameOU {
		assert.Equal(t, format.Total, gt.Market, "GameOU[%d] should have market %s, got %s", i, format.Total, gt.Market)
...
func validateProbability(t *testing.T, name string, prob format.Probability) {
	...
	assert.InDeltaf(t, 1.0, prob.ProbA+prob.ProbB, 0.01, "%s probabilities should sum to ~1.0, got %f", name, prob.ProbA+prob.ProbB)
}
...
func TestDeriveProbabilities(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := deriveProbabilities(tt.sim, tt.bestof)
			require.NoError(t, err, "deriveProbabilities returned unexpected error")
			assert.Equal(t, format.Moneyline, result.Moneyline.Market, "Expected moneyline market %s, got %s", format.Moneyline, result.Moneyline.Market)
...
func TestSimulationStructJSON(t *testing.T) {
	...
	assert.Equal(t, original.SimulationResult.Moneyline.ProbA, unmarshaled.SimulationResult.Moneyline.ProbA, "Moneyline ProbA mismatch")

	jsonString := string(jsonData)
	expectedFields := []string{"p1", "p2", "simulationResult", "Moneyline", "SetHandicaps", "GameHandicaps", "SetOU", "GameOU"}
...
func TestHandlerIntegration(t *testing.T) {
	t.Run("Full integration test", func(t *testing.T) {
		req := httptest.NewRequest("GET", "/?p1=0.6&p2=0.55&bestof=3", nil)
		w := httptest.NewRecorder()
		handler(w, req)
...
func BenchmarkDeriveProbabilities(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = deriveProbabilities(testSim, 3)
	}
}
	...
	tests := []struct {
		...
		},
		{
			...
			input: Simulation{
				...
				SimulationResult: SimulationResult{
					Moneyline:     format.Probability{Market: format.Moneyline, Line: "ml", ProbA: 0.6, ProbB: 0.4},
					SetHandicaps:  []format.Probability{{Market: format.Handicap, Line: "0.5", ProbA: 0.55, ProbB: 0.45}},
					GameHandicaps: []format.Probability{{Market: format.Handicap, Line: "2.5", ProbA: 0.52, ProbB: 0.48}},
...
func TestValidateProbability(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			prob:  format.Probability{ProbA: 0.6, ProbB: 0.4},
			valid: true,
		},
		{
			name:  "ProbA < 0",
			prob:  format.Probability{ProbA: -0.1, ProbB: 1.1},
			valid: false,
		},
		{
			name:  "ProbB > 1",
			prob:  format.Probability{ProbA: 0.5, ProbB: 1.2},
			valid: false,
		},
		{
			name:  "Sum not 1",
			prob:  format.Probability{ProbA: 0.7, ProbB: 0.1},
			valid: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.valid {
				validateProbability(t, tt.name, tt.prob)
			} else {
				defer func() {
					if r := recover(); r == nil {
						t.Errorf("Expected validation to fail for invalid probability, but it did not")
					}
				}()
				validateProbability(t, tt.name, tt.prob)
			}
